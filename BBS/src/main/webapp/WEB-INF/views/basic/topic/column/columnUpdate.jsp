	...
	<div class="container-fluid">
		<form class="form-horizontal registerform" action="${ctx}/basic/topic/column/columnSave.json">
			<div class="form-group">
				...
				<div class="col-sm-5">
					<input type="text" disabled="disabled" class="form-control" value="${column.number}" placeholder="栏目编号" />
					<input type="hidden" class="form-control" value="${column.number}" id="number" name="number" placeholder="栏目编号" />
				</div>
			</div>
			<div class="form-group">
				<label for="name" class="col-sm-2 control-label">上级栏目</label>
				<div class="col-sm-5">
					<input type="hidden" name="parentColumnId" id="parentColumnId" value="${column.column.id}">
					<input type="text" class="form-control" name="parentColumnName" value="${column.column.name}" id="parentColumnName" placeholder="上级栏目" />
			...
			<div class="form-group">
				...
				<div class="col-sm-5">
					<input type="text" class="form-control" value="${column.name}" id="name" name="name" placeholder="栏目名称" 
			...
			<div class="form-group">
				...
				<div class="col-sm-5">
					<select name="columnType" datatype="*1-18" nullmsg="请选择模块！" class="form-control" style="width: 441px;" id="columnType">
						...
						<option value="CMS" <c:if test="${column.columnType == 'CMS'}">selected="selected"</c:if> >CMS模块</option>
						<option value="COMMON" <c:if test="${column.columnType == 'COMMON'}">selected="selected"</c:if>>公共模块</option>
						<option value="NOTE" <c:if test="${column.columnType == 'NOTE'}">selected="selected"</c:if>>论坛模块</option>
					</select>
				</div>
				<div class="Validform_checktip"></div>
			</div>
